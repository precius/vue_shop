<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 角色列表区域 -->
      <el-table :data="roleslist" border stripe row-key="id" :tree-props="{ children: null, hashChildren: null }">
        <!-- 展开列 -->
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row v-for="(item1, i1) in scope.row.children" :key="item1.id" :class="['vcenter', 'bd-bottom', i1 === 0 ? 'bd-top' : '']">
              <!-- 渲染一级权限 -->
              <el-col :span="5">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEidtDialog(scope.row.id)">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    // 通过递归获取所有三级节点的id 并通过数组将所有id合在defKeys数组中
